...
// export default function Square({ square, row, col, piece, isSelected = false, isValidMove = false, onClick, onDrop, onDropOutside, }: SquareProps) {
//   const isDark = (row + col) % 2 === 1;
//   const className = ["square", isDark ? "dark" : "light", isSelected && "selected", isValidMove && "valid-move"].filter(Boolean).join(" ");
...
//   return (
//     <button type="button" className={className} onClick={onClick} onDragOver={handleDragOver} onDrop={handleDrop} draggable={!!piece} onDragStart={handleDragStart}>
//       {piece ? <Piece piece={piece} square={square} /> : null}
//       {isValidMove && <span className="move-dot" />}
//     </button>
//   );
// }
